...
set.seed(1)
use.many.seeds         <- TRUE
rand.seeds <- ifelse(use.many.seeds,121:130,1)
# read data
...
write.csv(test,"xgboost_preds_test_singleclass_2.csv",row.names = FALSE)
# write.csv(val,"xgboost_preds_val.csv",row.names = FALSE)
write.csv(val_future,"xgboost_preds_val_future_singleclass_2.csv",row.names = FALSE)
