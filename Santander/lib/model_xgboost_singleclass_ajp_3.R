...
set.seed(1)
use.many.seeds         <- TRUE
rand.seeds <- ifelse(use.many.seeds,141:150,1)
# read data
...
write.csv(test,"/u/project/miao/apryor/ml/xgboost_preds_test_singleclass_3.csv",row.names = FALSE)
# write.csv(val,"xgboost_preds_val.csv",row.names = FALSE)
write.csv(val_future,"/u/project/miao/apryor/ml/xgboost_preds_val_future_singleclass_3.csv",row.names = FALSE)
