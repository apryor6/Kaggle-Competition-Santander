...
purchase.frequencies <- create.lag.feature(as.data.table(purchase.frequencies),"num.transactions",1:11,na.fill=0)
for (window.size in 2:11){
  print(paste("Getting transaction count within last",window.size,"months"))
  colname <- paste("total.transactions.within.",window.size,"months",sep="")
  purchase.frequencies[[colname]]   <- 0
  for (month.ago in 1:window.size){
    current.col     <- paste("lagged.num.transactions.",month.ago,"months.ago",sep="")
    purchase.frequencies[[colname]]   <- purchase.frequencies[[colname]]  + purchase.frequencies[[current.col]]
  }

}
purchase.frequencies <- as.data.frame(purchase.frequencies)
